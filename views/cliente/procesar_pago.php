...
<style>
.checkout-wrap { background:#f4f6fb; min-height:100vh; padding:40px 0 60px; }
.checkout-title { font-size:26px; font-weight:800; color:#1a1a2e; margin-bottom:28px; }
.co-section { background:#fff; border-radius:16px; box-shadow:0 2px 12px rgba(0,0,0,.06); margin-bottom:24px; overflow:hidden; }
.co-section-header { background:#1a1a2e; color:#fff; padding:14px 24px; font-size:15px; font-weight:700; display:flex; align-items:center; gap:8px; }
.co-section-body { padding:24px; }

/* Tabla resumen */
.resumen-table { width:100%; border-collapse:collapse; }
.resumen-table th { font-size:12px; font-weight:700; text-transform:uppercase; letter-spacing:.5px; color:#6b7280; padding:0 0 12px; border-bottom:2px solid #f3f4f6; }
.resumen-table td { padding:14px 0; border-bottom:1px solid #f3f4f6; vertical-align:middle; }
.resumen-table tr:last-child td { border-bottom:none; }
.prod-img { width:52px; height:52px; object-fit:cover; border-radius:10px; margin-right:12px; flex-shrink:0; }
.prod-name { font-weight:600; font-size:14px; color:#1a1a2e; }
.prod-price { font-size:12px; color:#9ca3af; margin-top:2px; }
.resumen-total { display:flex; justify-content:space-between; align-items:center; padding:16px 24px; background:#f9fafb; border-top:2px solid #f3f4f6; }
.resumen-total span:first-child { font-size:14px; font-weight:600; color:#6b7280; text-transform:uppercase; letter-spacing:.5px; }
.resumen-total span:last-child { font-size:24px; font-weight:900; color:#7c3aed; }

/* Dirección */
.dir-alert { display:flex; align-items:flex-start; gap:14px; padding:18px 20px; background:#fff7ed; border:2px solid #fb923c; border-radius:16px; margin-bottom:0; }
.dir-alert strong { display:block; font-size:15px; color:#9a3412; margin-bottom:4px; }
.dir-alert p { font-size:13px; color:#c2410c; margin:0; }

/* Métodos */
.metodo-item { display:flex; align-items:center; justify-content:space-between; border:2px solid #e5e7eb; border-radius:14px; padding:16px 18px; margin-bottom:12px; cursor:pointer; transition:border-color .2s,background .2s,box-shadow .2s; gap:12px; }
.metodo-item:hover { border-color:#c4b5fd; box-shadow:0 4px 12px rgba(124,58,237,.08); }
.metodo-item:has(input[type="radio"]:checked) { border-color:#7c3aed; background:#faf5ff; box-shadow:0 4px 16px rgba(124,58,237,.12); }
.metodo-label { display:flex; align-items:center; gap:14px; flex:1; cursor:pointer; margin:0; }
.metodo-label input[type="radio"] { width:20px; height:20px; accent-color:#7c3aed; flex-shrink:0; }
.metodo-icon { font-size:28px; flex-shrink:0; }
.metodo-info { display:flex; flex-direction:column; gap:3px; }
.metodo-tipo { font-weight:800; font-size:13px; color:#1a1a2e; letter-spacing:.5px; text-transform:uppercase; }
.metodo-numero { font-size:15px; color:#374151; font-family:'Courier New',monospace; letter-spacing:2px; }
.metodo-titular { font-size:12px; color:#9ca3af; }
.metodo-acciones { display:flex; gap:8px; flex-shrink:0; }
.btn-mac { width:38px; height:38px; border:none; border-radius:10px; font-size:16px; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all .2s; }
.btn-mac-edit { background:#eff6ff; color:#2563eb; }
.btn-mac-edit:hover { background:#2563eb; color:#fff; }
.btn-mac-del { background:#fef2f2; color:#dc2626; }
.btn-mac-del:hover { background:#dc2626; color:#fff; }

/* Botón pagar */
.btn-pagar { width:100%; padding:16px; background:linear-gradient(135deg,#7c3aed,#6d28d9); color:#fff; border:none; border-radius:14px; font-size:18px; font-weight:800; cursor:pointer; margin-top:20px; transition:all .2s; box-shadow:0 4px 16px rgba(124,58,237,.3); }
.btn-pagar:hover { background:linear-gradient(135deg,#6d28d9,#5b21b6); transform:translateY(-2px); box-shadow:0 8px 24px rgba(124,58,237,.4); }
.btn-pagar:disabled { background:#e5e7eb; color:#9ca3af; cursor:not-allowed; box-shadow:none; transform:none; }

/* Formulario */
.form-label { font-size:13px; font-weight:700; color:#374151; margin-bottom:6px; display:block; }
.form-control,.form-select { border:2px solid #e5e7eb !important; border-radius:10px !important; padding:10px 14px !important; font-size:14px !important; transition:border-color .2s !important; background:#fafafa !important; }
.form-control:focus,.form-select:focus { border-color:#7c3aed !important; background:#fff !important; box-shadow:0 0 0 3px rgba(124,58,237,.1) !important; }
.btn-guardar { width:100%; padding:13px; background:#1a1a2e; color:#fff; border:none; border-radius:12px; font-size:15px; font-weight:700; cursor:pointer; transition:all .2s; margin-top:8px; }
.btn-guardar:hover { background:#7c3aed; transform:translateY(-1px); }
.seguridad-badge { display:flex; align-items:center; justify-content:center; gap:6px; margin-top:16px; font-size:12px; color:#9ca3af; font-weight:500; }
</style>
...
function abrirEditarDir(dir) {
    ...
    new bootstrap.Modal(document.getElementById('modalEditarDir')).show();
}
